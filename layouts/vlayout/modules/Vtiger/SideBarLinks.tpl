...
{strip}
    <div class="quickLinksDiv">
        {foreach item=SIDEBARLINK from=$QUICK_LINKS['SIDEBARLINK']}
            ...
            {if $MODULE eq 'Campaigns' && $DETAILVIEW_LINKS neq '' }
                <p class="{if $LINK_LIST_VIEW and $IS_LINK_MODULE_NAME}selectedQuickLink {else}unSelectedQuickLink{/if}"><a href="?module=Leads&view=Import&importid = {$RECORD->getId()}"><strong>Import Lead</strong></a>
                    </a></p>
            {/if}
            {if $MODULE eq 'Driver' && $DETAILVIEW_LINKS['DETAILVIEWBASIC'] neq ''}
                <p class="{if $LINK_LIST_VIEW and $IS_LINK_MODULE_NAME}selectedQuickLink {else}unSelectedQuickLink{/if}"><a href="driver_letter.php?driverid = {$RECORD->getId()}"><strong>Driver Letter</strong></a>
                    </a></p>
            {/if}
        {/foreach}
    </div>
