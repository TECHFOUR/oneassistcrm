...
jQuery.Class("Vtiger_Edit_Js",{
	...
	registerEventStatusChangeEvent : function(container){
        ...
        var followupContainer = container.find('.followUpContainer'); // Add for Call back with presentation
        container.find('select[name="activity_details"]').on('change',function(e){
            var selectedOption = jQuery(e.currentTarget).val();
            if(selectedOption == 'Call back with presentation' || selectedOption == 'Call back with no presentation' || selectedOption == 'Third Party Call back'){
                followupContainer.show();
...
// Add by Raghvender Singh for Call back assistance
	...
	registerBasicEvents : function(container) {
        ...
        /*Start code by Raghvender Singh on 16052014*/
        this.newChangePlanEvent(container);
        /*End code by Raghvender Singh on 16052014*/
...
// Add by Raghvender Singh for Change Plan in Events Mode check box on 14052014
    newChangePlanEvent : function(container) {

        var Change_plan_block = container.find('.Change_plan_block');

        container.find('input[name="change_plan"]').on('change',function(e){

            if($('#Events_editView_fieldName_change_plan').attr('checked')) {
                Change_plan_block.show();
            } else {
                Change_plan_block.hide();
            }
        });
    },
// End Add by Raghvender Singh for Change Plan in Events Mode check box14052014
