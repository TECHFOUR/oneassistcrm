...
{if $MODULE eq 'Leads' && $FIELD_MODEL->getFieldName() eq 'final_status'}
...
<input id="{$MODULE}_editView_fieldName_{$FIELD_NAME}" type="text" 
	   ...
	   value="{$FIELD_MODEL->get('fieldvalue')}" readonly="readonly"
...
{else if $MODULE eq 'Leads' && $FIELD_MODEL->getFieldName() eq 'mobile'}

<input id="{$MODULE}_editView_fieldName_{$FIELD_NAME}" type="text"  maxlength="10" minlength="9"
	   class="input-large {if $FIELD_MODEL->isNameField()}nameField{/if}" 
	   data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true}required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" 
	   name="{$FIELD_MODEL->getFieldName()}" 
