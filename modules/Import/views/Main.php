...
//			}
	...
	public static function showResult($importInfo, $importStatusCount) {
		...
		// Code Added by Raghvender Singh on 17052014
		if($moduleName == 'Leads'){
			$file_path = $_SESSION['log_error_path'];
			$numberOfRecordsRead = $_SESSION['numberOfRecordsRead'];
			$updated_records = $_SESSION['updated_records'];
			$skipped_records = $_SESSION['skipped_records'];
			$imported_records = $_SESSION['imported_records'];
			unset($_SESSION['updated_records']);
			unset($_SESSION['skipped_records']);
			unset($_SESSION['imported_records']);
			unset($_SESSION['numberOfRecordsRead']);
		...
		// Code Added by Raghvender Singh on 17052014
		if($moduleName == 'Leads'){
			$viewer->assign('numberOfRecordsRead',$numberOfRecordsRead);
			$viewer->assign('updated_records',$updated_records);
			$viewer->assign('skipped_records', $skipped_records);
			$viewer->assign('imported_records', $imported_records);
			$viewer->assign('file_path', $file_path);
			$viewer->assign('current_path', $current_path);
