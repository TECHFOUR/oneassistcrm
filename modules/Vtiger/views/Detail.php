...
class Vtiger_Detail_View extends Vtiger_Index_View {
	...
	function showModuleSummaryView($request) {
        ...
        //Add for direct URL on add button in activity ADD button  by Raghvender Singh on 14052014
        $CurrentUrl = "http://".$_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"];

        $url2=explode('/', $CurrentUrl);
        $url=array_pop($url2);
        $url2=implode('/', $url2);
        $CreatedURL = $url2.'/index.php?module=Leads&view=Detail&record='.$recordId;
        //End for direct URL on add button in activity ADD button  by Raghvender Singh on 14052014

        $viewer = $this->getViewer($request);
        //Assign for direct URL on add button in activity ADD button  by Raghvender Singh on 14052014
        $viewer->assign('CURRENTURL', $CurrentUrl);
        $viewer->assign('CREATEDURL', $CreatedURL);
