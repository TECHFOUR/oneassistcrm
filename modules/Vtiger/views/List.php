...
class Vtiger_List_View extends Vtiger_Index_View {
	...
	function preProcess(Vtiger_Request $request, $display=true) {
		...
		// Start Query for User by Raghvender Singh on 12052014
		$user_qry = $adb->query("SELECT * FROM vtiger_users WHERE vtiger_users.id = ".$user_id."");
		//echo "<pre>"; print_r($user_qry); die("ram");				
						
		 if($adb->num_rows($user_qry) > 0) {
           while($row = $adb->fetch_array($user_qry)){
                $user_id = $row['id'];
				
		   }
		 }
		// End Query for User by Raghvender Singh on 12052014		
